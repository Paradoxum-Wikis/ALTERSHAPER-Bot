...
export class ModerationLogger {
  private static readonly MOD_FILE = path.join(
    process.cwd(),
    "data",
    "moderation.json",
  );
  ...
  public static async addEntry(
    entry: Omit<ModerationEntry, "id" | "timestamp">,
  ): Promise<string> {
    const entries = await this.readEntries();
  ...
  public static async getUserEntries(
    ...
    return entries.filter(
      (entry) =>
        entry.userId === userId && entry.guildId === guildId && !entry.mercy,
    );
  }

  public static async getUserWarns(
    userId: string,
    guildId: string,
  ): Promise<ModerationEntry[]> {
    const entries = await this.readEntries();
  ...
  public static async removeEntry(entryId: string): Promise<boolean> {
    ...
    const index = entries.findIndex(
      (entry) => entry.id === entryId && !entry.mercy,
    );
