...
export async function execute(interaction: ChatInputCommandInteraction, executor: GuildMember): Promise<void> {
  if (!executor.permissions.has(PermissionFlagsBits.BanMembers)) {
    await interaction.reply({ content: '**THOU EGO LACKEST THE SUPREME AUTHORITY TO DELIVER ETERNAL JUDGEMENT!**', ephemeral: true });
    return;
  }

  const targetUser = interaction.options.getUser('user')!;
  const reason = interaction.options.getString('reason') || 'HERETICAL DEFIANCE OF ALTERUISM';
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`**${targetUser} HATH BEEN CAST INTO THE VOID FOR HERETICAL DEFIANCE!**`)
      .addFields(
        { name: 'EXECUTOR OF DIVINE WILL', value: `${executor.user.tag}`, inline: true },
        { name: 'ACTION ID', value: `${entryId}`, inline: true },
        { name: 'REASON FOR ETERNAL PUNISHMENT', value: reason, inline: false },
        { name: 'SACRED DECREE', value: 'THOSE WHO DEFY ALTERUISM SHALL KNOW ETERNAL EXILE', inline: false }
      )
      .setTimestamp();

    await interaction.reply({ embeds: [embed] });
  } catch (error) {
    await interaction.reply({ content: '**THE DIVINE POWERS HAVE BEEN THWARTED! THE HERETIC REMAINS BEYOND REACH!**', ephemeral: true });
  }
}