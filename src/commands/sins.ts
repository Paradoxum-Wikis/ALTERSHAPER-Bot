...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    if (targetUser) {
      ...
      const userEntries = await ModerationLogger.getUserEntries(targetUser.id, interaction.guild.id);
      
      if (userEntries.length === 0) {
        const embed = new EmbedBuilder()
          .setColor('#00FF00')
          .setTitle('📋 RECORDS OF THE DAMNED')
          .setDescription(`**${targetUser.tag} HATH A CLEAN SLATE!**\n\nNO SINS RECORDED IN THE RECORDS OF THE DAMNED.`)
      ...
      if (userEntries.length > 10) {
        embed.setFooter({ text: `SHOWING LATEST 10 OF ${userEntries.length} ENTRIES` });
      }
      ...
      const allEntries = await ModerationLogger.getAllEntries(interaction.guild.id);
      
      if (allEntries.length === 0) {
        const embed = new EmbedBuilder()
          .setColor('#00FF00')
          .setTitle('📊 HOLY STATISTICS OF RIGHTEOUSNESS')
          .setDescription('**THE SACRED HALLS REMAIN PURE!**\n\nNO TRANSGRESSIONS HAVE BEEN RECORDED IN THIS REALM.')
          .setTimestamp();
    ...
    await interaction.reply({ content: '**THE DIVINE POWERS HAVE FAILED TO RETRIEVE THE RECORDS OF THE DAMNED!**', ephemeral: true });
  }
}