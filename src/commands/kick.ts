...
export async function execute(interaction: ChatInputCommandInteraction, executor: GuildMember): Promise<void> {
  if (!executor.permissions.has(PermissionFlagsBits.KickMembers)) {
    await interaction.reply({ content: '**THOU EGO LACKEST THE DIVINE AUTHORITY TO CAST OUT THE FAITHLESS!**', ephemeral: true });
    return;
  }

  const targetUser = interaction.options.getUser('user')!;
  const reason = interaction.options.getString('reason') || 'DEFIANCE OF SACRED ALTERUISM';

  const targetMember = interaction.guild?.members.cache.get(targetUser.id);
  if (!targetMember) {
    await interaction.reply({ content: '**THE FAITHLESS ONE HATH ALREADY FLED FROM OUR SACRED HALLS!**', ephemeral: true });
    return;
  }
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`**${targetUser} HATH BEEN CAST OUT FOR DEFYING THE SACRED LAW OF ALTERUISM!**`)
      .addFields(
        ...
        { name: 'HOLY DECREE', value: 'THE FAITHLESS SHALL NOT DWELL AMONG THE RIGHTEOUS', inline: false }
      )
      .setTimestamp();

    await interaction.reply({ embeds: [embed] });
  } catch (error) {
    await interaction.reply({ content: '**THE DIVINE POWERS HAVE BEEN THWARTED! THE TARGET REMAINS BEYOND REACH!**', ephemeral: true });
  }
}