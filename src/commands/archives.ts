...
export async function execute(interaction: ChatInputCommandInteraction, executor: GuildMember): Promise<void> {
  ...
  if (!executor.permissions.has(PermissionFlagsBits.ManageMessages)) {
    await interaction.reply({ content: '**THOU EGO LACKEST THE AUTHORITY TO ACCESS THE SERAPHIC ARCHIVES!**', ephemeral: true });
    return;
  }
  ...
  try {
    ...
    if (actionId) {
      ...
      purgedMessages = await MessageLogger.getPurgedMessagesByActionId(actionId);
      ...
      purgedMessages = await MessageLogger.getPurgedMessages(interaction.guild.id);
    ...
    if (purgedMessages.length > 10) {
      embed.setFooter({ text: `SHOWING LATEST 10 OF ${purgedMessages.length} ARCHIVED MESSAGES` });
    }

    embed.addFields({
      name: 'SACRED REMINDER',
      value: 'THESE ARCHIVES ARE FOR MODERATION PURPOSES ONLY. PLEASE HANDLE WITH DISCRETION.',
      inline: false
    });
    ...
    await interaction.reply({ content: '**THE DIVINE POWERS HAVE FAILED TO ACCESS THE ARCHIVES!**', ephemeral: true });
  }
}