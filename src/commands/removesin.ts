...
export const data = new SlashCommandBuilder()
  .setName('removesin')
  .setDescription('ABSOLVE A SOUL OF ITS RECORDED TRANSGRESSION AND UNDO THE PUNISHMENT')
...
export async function execute(interaction: ChatInputCommandInteraction, executor: GuildMember): Promise<void> {
  if (!executor.permissions.has(PermissionFlagsBits.KickMembers)) {
    await interaction.reply({ content: '**THOU EGO LACKEST THE AUTHORITY TO GRANT DIVINE ABSOLUTION!**', ephemeral: true });
    return;
  }
  ...
  try {
      ...
      switch (entry.type) {
        case 'timeout':
          ...
          if (timeoutMember.isCommunicationDisabled()) {
            await timeoutMember.timeout(null, `Timeout removed by ${executor.user.tag} (Mercy granted for ${entryId})`);
        ...
        case 'ban':
          try {
            ...
            if (ban) {
              await interaction.guild.members.unban(entry.userId, `Ban removed by ${executor.user.tag} (Mercy granted for ${entryId})`);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`**ENTRY ${entryId} HATH BEEN PARDONED WITH DIVINE MERCY!**`)
      .addFields(
        { name: 'ORIGINAL TRANSGRESSION', value: `${entry.type.toUpperCase()} AGAINST ${targetUser}`, inline: true },
        { name: 'GRANTOR OF MERCY', value: `${executor.user.tag}`, inline: true },
        { name: 'PUNISHMENT STATUS', value: undoResult, inline: false },
        { name: 'DIVINE DECREE', value: 'THE SIN IS PARDONED AND PUNISHMENT LIFTED WHERE POSSIBLE. GO FORTH, AS YOU MAY UNLEASH YOUR ALTER EGO ONCE AGAIN!', inline: false }
      )
      .setTimestamp();
