...
export async function execute(
  ...
  try {
    ...
    const confirmationEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          ...
          value: "React with 🖋️ within 1 minute to proceed with the severance ritual.",
          inline: false,
        }
      )
    ...
    try {
      ...
      if (collected.size > 0) {
        ...
        if (targetMember) {
          ...
          const rolesToRemoveFiltered = rolesToRemove.filter(roleId => targetMember.roles.cache.has(roleId));
      ...
      const timeoutEmbed = new EmbedBuilder()
        ...
        .setDescription("**THE SEVERANCE CONFIRMATION HAS EXPIRED. NO CHANGES WERE MADE.**")
        .addFields({
          name: "TO RETRY",
          value: "Run the `/unlink` command again to restart the severance process.",
          inline: false,
        });
    ...
    await interaction.reply({
      content: "**A DISTURBANCE IN THE SACRED HALLS! The severance ritual failed. The oracles are perplexed. Try again later, or consult the high scribes.**",
      flags: MessageFlags.Ephemeral,
    });
  }
}